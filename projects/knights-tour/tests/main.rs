...
fn test_knights_tour() {
    ...
    for (index, state) in knights.initial_state().warnsdorff_rule().take(1).enumerate() {
...
#[test]
fn test_pawns_tour() {
    let knights = Chessboard::new(5, 5).with_role(ChessRole::Pawn).walk(false);
    std::fs::create_dir_all("target/").unwrap();
...
impl Board {
    fn new() -> Self {
        Self { field: [[0; SIZE]; SIZE] }
    }

    fn available(&self, p: Point) -> bool {
        0 <= p.x && p.x < SIZE as i32 && 0 <= p.y && p.y < SIZE as i32 && self.field[p.x as usize][p.y as usize] == 0
    }
